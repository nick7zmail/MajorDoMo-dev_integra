...
*/
  ...
  $table_name='dev_integra_devices';
  $rec=SQLSelectOne("SELECT * FROM $table_name WHERE ID='$id'");
  if ($this->mode=='learn') {
	  //тут переход в режим обучения
  }
  if ($this->mode=='update') {
   $ok=1;
  // step: default
  ...
  //updating 'DEV_TYPE' (varchar)
   global $dev_type;
   $rec['DEV_TYPE']=$dev_type;
  //updating 'DEV_CONTROL' (varchar)
   global $dev_control;
   $rec['DEV_CONTROL']=$dev_control;
  ...
  if ($this->tab=='data') {
   ...
   if ($this->mode=='update') {
	...
	if ($title_new) {
	 ...
	 $new_id=SQLInsert('dev_integra_commands',$prop);
	}
   }
   global $delete_id;
   if ($delete_id) {
    SQLExec("DELETE FROM dev_integra_commands WHERE ID='".(int)$delete_id."'");
   }
   $properties=SQLSelect("SELECT * FROM dev_integra_commands WHERE DEVICE_ID='".$rec['ID']."' ORDER BY ID");
   ...
   for($i=0;$i<$total;$i++) {
    ...
    if ($this->mode=='update') {
      ...
      SQLUpdate('dev_integra_commands', $properties[$i]);
